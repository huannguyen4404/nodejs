...
class UsersMiddleware {
  ...
  async validateExistedEmail(req: express.Request, res: express.Response, next: express.NextFunction) {
  ...
  async validateEmailBelongSameUser(req: express.Request, res: express.Response, next: express.NextFunction) {
    const user = await userService.getUserByEmail(req.body.email);
  ...
  async validatePatchEmail(req: express.Request, res: express.Response, next: express.NextFunction) {
    if (req.body.email) {
      UsersMiddleware.getInstance().validateEmailBelongSameUser(req, res, next);
    } else {
      next();
    }
  }
